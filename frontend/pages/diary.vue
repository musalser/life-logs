...
const savedLabel = computed(() => {
  if (!lastSavedAt.value) return 'Не сохранено'
  try {
...
const save = async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Не удалось сохранить страницу')
    ...
    if (String(route.query.page || '') !== String(savedPage.id)) {
      await router.replace({ name: 'diary', query: { page: String(savedPage.id) } })
    }
