        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex items-center gap-4">
            ...
            <nav class="flex items-center gap-2">
              <RouterLink :to="{ name: 'chat' }" class="nav-link" active-class="nav-link--active">Чат</RouterLink>
              <RouterLink :to="{ name: 'diary' }" class="nav-link" active-class="nav-link--active">Дневник</RouterLink>
            </nav>
          </div>
    ...
    >
      <section class="w-full max-w-md rounded-2xl border border-app-border/50 bg-app-panel p-6 shadow-2xl">
        ...
        <form class="space-y-3" @submit.prevent="submitAuth">
          ...
          <p v-if="authError" class="text-sm text-app-error">{{ authError }}</p>

          <div class="mt-2 flex items-center justify-between gap-2">
            <button type="button" class="auth-secondary" @click="closeAuthModal">Отмена</button>
            <button type="submit" class="auth-primary" :disabled="authLoading">
...
const isAuthenticated = computed(() => Boolean(store.state.authToken));
const authUser = computed(() => store.state.authUser || 'Пользователь');
...
const loadDiaryPages = async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/diary/pages?limit=100&offset=0`, {
      headers: {
...
const submitAuth = async () => {
  try {
    ...
    // Error text is managed by store.authError.
  }
};
...
const handleDeleteDiaryPage = async (pageId) => {
  if (!confirm('Вы уверены? Эту страницу нельзя будет восстановить.')) {
    return;
  }
  ...
  try {
    ...
    // Перезагружаем список и закрываем страницу если она открыта
...
watch(
  () => [route.name, store.state.authToken],
  async () => {
    await loadDiaryPages();
  },
  { immediate: true }
